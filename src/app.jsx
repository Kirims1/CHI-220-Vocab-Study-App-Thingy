  ...
  const buildDeck = () => {
    ...
    selectedUnits.forEach(unit => {
      if (studyVocabulary && masterQuizList[unit]?.vocab) {
        deck = [...deck, ...masterQuizList[unit].vocab.map(card => ({ ...card, unit }))];
      }
      if (studyQuizList) {
        deck = [...deck, ...(masterQuizList[unit]?.dialogue || []).map(card => ({ ...card, unit })), ...(masterQuizList[unit]?.reading || []).map(card => ({ ...card, unit }))];
      }
    });
    return deck;
  };
        ...
        <div className="bg-white p-8 md:p-10 rounded-3xl shadow-2xl ring-1 ring-slate-900/5 max-w-4xl w-full border-t-4 border-indigo-500">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-left">
              {unitEntries.map(([unitKey, data], index) => {
                ...
                const stats = lessonStats[unitKey];
                const progressLabel = stats
                  ? `${Math.round((stats.correct / stats.attempted) * 100)}% correct · ${stats.attempted} answered`
                  : 'Not started';
                return (
                  <label
                    key={unitKey}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className={`mt-1 text-[10px] font-extrabold uppercase tracking-wider ${stats ? 'text-slate-600' : 'text-slate-400'}`}>{progressLabel}</p>
                    </div>
          ...
          <div className="mb-8 text-left">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className={`group relative flex items-start cursor-pointer p-5 rounded-2xl border-2 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.98] ${studyVocabulary ? 'border-indigo-500 bg-gradient-to-br from-indigo-50 to-violet-100 shadow-md shadow-indigo-100' : 'border-slate-200 bg-white hover:border-indigo-200 hover:bg-indigo-50/50 hover:shadow-sm'}`}>
                  <input
                    type="checkbox"
                    className="sr-only"
                  ...
                  />
                  <span className={`mr-4 mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${studyVocabulary ? 'border-indigo-500 bg-white text-indigo-700' : 'border-slate-300 bg-white text-transparent'}`}>
                    <CheckCircle2 size={16} strokeWidth={3} />
                  </span>
                  <div>
                  ...
                  {studyVocabulary && <span className="absolute right-4 top-4 text-[10px] font-extrabold uppercase tracking-wider text-indigo-700">Selected</span>}
                </label>
                <label className={`group relative flex items-start cursor-pointer p-5 rounded-2xl border-2 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.98] ${studyQuizList ? 'border-emerald-500 bg-gradient-to-br from-emerald-50 to-teal-100 shadow-md shadow-emerald-100' : 'border-slate-200 bg-white hover:border-emerald-200 hover:bg-emerald-50/50 hover:shadow-sm'}`}>
                  ...
                  />
                  <span className={`mr-4 mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${studyQuizList ? 'border-emerald-500 bg-white text-emerald-700' : 'border-slate-300 bg-white text-transparent'}`}>
                    <CheckCircle2 size={16} strokeWidth={3} />
                  </span>
                  <div>
                  ...
                  {studyQuizList && <span className="absolute right-4 top-4 text-[10px] font-extrabold uppercase tracking-wider text-emerald-700">Selected</span>}
                </label>
             </div>
          </div>
